...
import { Main1container, LogoWrapper, LogoImage, MenuIcon } from "../styles/SocialMining.styles";
  ...
  return (
    <>
      <Main1container>
        ...
        <MainContaier>
          <Heading>{showMenu ? 'Connect your Social accounts' : DashBoardTitle}</Heading>
        </MainContaier>
        <MenuIcon onClick={toggleMenu} >{showMenu ? <img src={closeIcon}/>:<img src={menuIcon}/>}</MenuIcon>
      
      </Main1container>

      {showMenu ? (<SocialConnectModel/>):
      (<Outlet/>)}
    </>
  );
};
